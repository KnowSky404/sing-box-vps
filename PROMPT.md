...
功能加固：在 generate_config 中增加对 jq 的强制存在性检查。由于当前的配置生成完全依赖 jq 的安全变量注入，该改动确保了在缺失 jq 的环境下脚本能自动进行自我修复安装。

Bug大发现：终于定位 Base64 解析失败（byte 40）的罪魁祸首！在从文件中读取包含 '=' 字符的密钥时，cut -d'=' -f2 命令把末尾用于补位的 '=' 当作了分隔符直接丢弃，导致 Base64 字符串长度从 44 位被截断为 43 位。现已全部修正为 cut -d'=' -f2-，完美保留完整密钥。

Bug修复：修复在 Warp 管理或修改配置时 Short ID 被重新随机生成导致客户端连接失效的问题。现在会在重新生成配置前优先提取并保留原有的 Short ID。

文档更新：全面重构 README.md，重点增加 Cloudflare Warp 解锁功能、1.13.x Endpoint 架构说明以及 jq 安全配置生成特性的展示。
