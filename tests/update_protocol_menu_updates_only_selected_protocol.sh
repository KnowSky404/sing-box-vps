...
if ! grep -Fq 'PORT=9443' "${SB_PROTOCOL_STATE_DIR}/hy2.env"; then
  printf 'expected hy2 state file to update port, got:\n%s\n' "$(cat "${SB_PROTOCOL_STATE_DIR}/hy2.env")" >&2
  exit 1
fi

if ! grep -Fq 'PASSWORD=new-pass' "${SB_PROTOCOL_STATE_DIR}/hy2.env"; then
  printf 'expected hy2 state file to update password, got:\n%s\n' "$(cat "${SB_PROTOCOL_STATE_DIR}/hy2.env")" >&2
  exit 1
fi
