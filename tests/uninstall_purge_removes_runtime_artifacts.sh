...
if [[ -e "${SBV_BIN_PATH}" ]]; then
  printf 'expected sbv binary to be removed: %s\n' "${SBV_BIN_PATH}" >&2
  exit 1
fi
...
if ! grep -Fxq 'stop sing-box' "${TMP_DIR}/systemctl.log"; then
  printf 'expected systemctl stop sing-box to be called\n' >&2
  exit 1
fi

if ! grep -Fxq 'disable sing-box' "${TMP_DIR}/systemctl.log"; then
  printf 'expected systemctl disable sing-box to be called\n' >&2
  exit 1
fi

if ! grep -Fxq 'daemon-reload' "${TMP_DIR}/systemctl.log"; then
  printf 'expected systemctl daemon-reload to be called\n' >&2
  exit 1
fi
